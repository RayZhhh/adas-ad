...
_CAST_SCHEDULE_TASK_DESCRIPTION = '''\
We assume that there are a total of $N$ objects to be made with a demand of exactly $r_j$ (> 0, an integer) copies for $j$-th object. \
Each object has a fixed weight $w_i$ in kg, thereby requiring a total of $M=\sum{_{j=1}^N}r_j w_j$ kg of metal to make all copies of $j$-th object. \
Without loss of generality, let us also assume that $W_i$ kg of metal is melted at i-th heat, \
thereby allowing us to consider a different crucible size at each heat. \
Then, the total number of heats ($H$) required to melt the above metal. \
The solution X is a (H, N) shaped matrix, where X[i, j] indicates the number of objects j built in melt i. \
The solution X is a better solution if it has a higher metal utilization and does not violate the constraints.
...
CAST_SCHEDULE_TASK_DESCRIPTION = '''\
We decide to use a genetic algorithm (GA) framework to optimize the solution. 
The most crucial part of our GA is to design an effective `gen_new_population` function, which performs selection, recombination, mutation, and survival and return a new population.
An example of `gen_new_population` function is shown below. Your task is to design a more effective `gen_new_population` function. 
Please note that the argument is fixed and do not modify it in your implementation, and do not import additional Python packages.
...
Role_Assignment = {"thought": "Similar to Auto-GPT and expert prompting, we can use dynamic control flow in the design to let the agent decide what expert we should use.",
        ...
        # Instruction for step-by-step reasoning
        cot_instruction = "Please think step by step and then solve the task."
        expert_agents = [LLMAgentBase(['thinking', 'answer'], 'Expert Agent', role=role) for role in ['Computer Science Expert', 'Python Coding Expert', 'Algorithm Design Expert', 'Physics Expert', 'Chemistry Expert', 'Biology Expert', 'Science Generalist']] 
        ...
        if 'computer' in choice.content.lower():
            expert_id = 0
        elif 'python' in choice.content.lower():
            expert_id = 1
        elif 'algorithm' in choice.content.lower():
            expert_id = 2
        if 'physics' in choice.content.lower():
            ...
            expert_id = 6 # Default to Science Generalist
