...
from cast_schedule_prompt import get_init_archive, get_prompt, get_reflexion_prompt
from evaluate import EvalPxy
from alevo.base import SecureEvaluator
from alevo.base import TextFunctionProgramConverter as TFPC
from alevo.tools.profiler import WandBProfiler

# client = openai.OpenAI()
client = openai.OpenAI(
    api_key="sk-82f35efa54294fbbb0849e555784841e",
...
class LLMAgentBase:
    ...
    def generate_prompt(self, input_infos, instruction):
        ...
        output_fields_and_description = {
            key: f"Your {key}." if not 'answer' in key else f"Your {key}. Return ONLY the alphabet choice, i.e. A or B or C or D."
...
def search(args):
    ...
    for solution in archive:
        ...
        # RZ: 利用之前的 fitness 计算 bootstrap confidence interval
...
evaluator = EvalPxy(timeout_seconds=500)
evaluator = SecureEvaluator(evaluator)


# wandb.login(key='579d2d16d85fdeabe12fac0f5eccf903a2c594da')
# profiler = WandBProfiler(log_dir='logs/run1', wandb_project_name='adas_run1', name='run1')


def evaluate(args):
    file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive.json")
    eval_file_path = str(os.path.join(args.save_dir, f"{args.expr_name}_run_archive.json")).strip(
        ".json") + "_evaluate.json"
    with open(file_path, 'r') as json_file:
        archive = json.load(json_file)
    eval_archive = []
...
def evaluate_forward_fn(args, forward_str):
    # -------- RZ: `compile` the code of the forward function and set it to `AgentSystem` class --------
    # -------- RZ: our llm4ad package provide more efficient APIs --------------------------------------
    # -------- RZ: leave an unimplemented `forward` function in `AgentSystem` class to enable hints ----
    namespace = {}
    ...
    # get the code generated by the agent system
    from cast_schedule_prompt import CAST_SCHEDULE_TASK_DESCRIPTION
    question = Info('task', 'User', CAST_SCHEDULE_TASK_DESCRIPTION, -1)
    agent_sys = AgentSystem()
    code = agent_sys.forward(question)
    print(code)
    exit()

    from cast_schedule_prompt import TEMPLATE_PROGRAM
    from alevo.base import SamplerTrimmer
    code = SamplerTrimmer.auto_trim(code)
    code = SamplerTrimmer.sample_to_program(code, TEMPLATE_PROGRAM)


def evaluate_forward_fn_(args, forward_str):
